...
int VulkanManager::vpCenterX = 0;
int VulkanManager::vpCenterY = 0;
...
//	init_glslang();
	...
	while (true)//每循环一次绘制一帧画面
	{
		...
		viewportList[0].x = float(vpCenterX - screenWidth / 4);					//视口X坐标
		viewportList[0].y = float(vpCenterY - screenHeight / 4);					//视口Y坐标
		viewportList[0].width = float(screenWidth / 2);			                //视口宽度
		viewportList[0].height = float(screenHeight / 2);			            //视口高度
