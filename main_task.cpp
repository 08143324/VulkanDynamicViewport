...
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_LBUTTONDOWN://鼠标左键按下
		...
		VulkanManager::vpCenterX = preX;			//设置vpCenterX为触控点x坐标
		VulkanManager::vpCenterY = preY;			//设置vpCenterY为触控点y坐标
		if (preX>1366 / 4 * 3)		//判断触控点x坐标是否大于允许的最大值
			VulkanManager::vpCenterX = 1024;
		if (preX<1366 / 4)		//判断触控点x坐标是否小于允许的最小值
			VulkanManager::vpCenterX = 341;
		if (preY>768 / 4 * 3)		//判断触控点y坐标是否大于允许的最大值
			VulkanManager::vpCenterY = 576;
		if (preY<768 / 4)		//判断触控点y坐标是否小于允许的最小值
			VulkanManager::vpCenterY = 192;
		mouseLeftDown = true;
		break;
	case WM_LBUTTONUP:
		mouseLeftDown = false;
